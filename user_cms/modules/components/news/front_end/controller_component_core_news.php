...
class controller_component_core_news extends component {
	...
	public function action_index() {
		...
		$params = array(
			...
			'join' => 'category_url',
			'count' => $this->component_config['main_page_count']
		);
			//'limit' => $this->component_config['main_page_count']
		$params['page'] = isset($this->url['params']['page']) ? $this->url['params']['page'] : 1; //страница в пагинации
		$this->data['page'] = $params['page']; //текущая страница
		$this->data['count_pages'] = $this->model->get_count_pages('', $params['count']); //количество страниц в пагинации
	...
	public function action_else() {
		...
		if($category) {
			...
			if(!isset($this->url['actions'][1])) {
				...
				//$this->data['category_text'] = $category['text'];
				
				$params = array(
					'type' => 'by_category',
					'count' => $this->component_config['index_page_count'], //количество новостей на странице
				);
				$params['page'] = isset($this->url['params']['page']) ? $this->url['params']['page'] : 1; //страница в пагинации
				$this->data['page'] = $params['page']; //текущая страница
				$this->data['count_pages'] = $this->model->get_count_pages($category['url'], $params['count']); //количество страниц в пагинации
				$this->data['news_items'] = array();
