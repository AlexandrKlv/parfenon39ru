...
*/
class controller_plugin_feedback_prot extends plugin
{	
	...
	public function action_index($plugin) {
		...
		$this->data['src_captcha'] = SITE_URL . '/user_cms/helpers/captcha.php';
		if ($params['fields']) {
			...
			$this->page['head'] = $this->add_css_file(SITE_URL . '/modules/plugins/' . $this->plugin_name . '/views/style.css');
	...
	private function validate($fields) {
		...
		// $res=json_decode(file_get_contents('https://www.google.com/recaptcha/api/siteverify?secret=6LepbwATAAAAAE7VFKoHtqgHHV09ocd6-P6EdFw0&response='.$_POST['g-recaptcha-response']));
		foreach ($fields as $i => $field) {
			if (isset($field['required']) && $field['required']) {
				...
				if (empty($field['error_message'])) {
					$field['error_message'] = 'Некорректно заполнено поле "' . $field['label'] . '"';
				}
			...
			if (($field['type'] == 'captcha')&&((!isset($_SESSION['captcha']) || !isset($_POST['captcha']) || $_SESSION['captcha'] != $_POST['captcha']))) {$this->errors[$i] = "Некорректно заполнено поле капча";}
		}
