...
<div>
		...
		<select name="tipob" id="tipob">
				<option value="1" <?php if (isset($type_id)) if ($type_id==1) { ?>selected<?php } ?>>Продам</option>
				<option value="2" <?php if (isset($type_id)) if ($type_id==2) { ?>selected<?php } ?>>Сдам</option>			
		</select>
</div><div>		
		<h3>Тип объекта (рубрики)</h3>
		<?php if (!empty($types)) foreach ($types as $type) { ?>
		   <input type="radio" name="tip" value="<?php echo $type['id']; ?>" <?php if ($type_id==$type['id']) { ?>checked<?php } ?>><?php echo $type['name']; ?><Br>
		<?php } ?>
</div><div>		
		<h3>Расположение</h3>
		<select name="resp1" id="resp1">
			<?php if (!empty($cities)) foreach ($cities as $city) { ?>
				<option value="<?php echo $city['id']; ?>" <?php if (isset($city_id)) if ($city_id==$city['id']) { ?>selected<?php } ?>><?php echo $city['name']; ?></option>
			<?php } ?>
		</select>
</div><div>		
		<h3>Районы</h3>
		<div id="rayons2">
		   ...
		   <input type="radio" name="rayonx" value="<?php echo $district['id']; ?>" <?php if (isset($district_id)) if ($district_id==$district['id']) { ?>checked<?php  } ?>><?php echo $district['name']; ?><Br>
...
<div>		
		...
		<?php if (!empty($numRooms)) foreach ($numRooms as $numRoom) if ($numRoom['id']!=5) { ?>
		   <!--input type="checkbox" name="komn<?php echo $numRoom['id'];?>" value="<?php echo $numRoom['id'];?>" <?php if (isset($realtyChecked['num_rooms'][$numRoom['id']])) { ?>checked<?php } ?>><?php echo $numRoom['name'];?><Br-->
				<option value="<?php echo $numRoom['id'];?>" <?php if (isset($num_rooms_id)) if ($num_rooms_id==$numRoom['id']) { ?>selected<?php } ?>><?php echo $numRoom['name'];?></option>
		<?php } ?>
		</select>
</div><div>		
		<h3>Тип дома</h3>
		...
		<?php if ($imp>0) { ?>
		<h3>Сортировка (1 - самый первый)</h3>
		<select name="impo">
			<?php for ($j=1; $j<11; $j++) {?>
			<option value="<?php echo $j;?>" <?php if ($impo==$j) { ?>selected<?php } ?>><?php echo $j;?></option>
			<?php } ?>
		</select>
		<?php } ?>
